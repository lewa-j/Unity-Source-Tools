...
public class COLLADAExport : MonoBehaviour
{
	...
	public static void Export (string path, Geometry geom, bool zUp, bool sourceUnits)
	{
		...
		WriteXmlDocument (path, geom, zUp, sourceUnits);
	...
	static void WriteXmlDocument (string path, Geometry geom, bool zUp, bool sourceUnits)
	{
	...
	{
		print ("writing geometry library");
		print (geoms.Length + " geoms.Length");
		for (int i = 0; i < geoms.Length; i++)
		{
			XmlNode Geom = xDoc.CreateElement ("geometry");
			GeomLib.AppendChild (Geom);
				print ("writing geometry library inside for loop");
			WriteGeometry (geoms[i], Geom, xDoc, path);
			print ("writing geometry library inside for loop is done");
	...
	static XmlNode WriteGeometry (Geometry geom, XmlNode GeomNode, XmlDocument xDoc, string path)
	{
		...
		print ("about to write mesh/geometry");
	...
	static XmlNode WriteMesh (COLLADAMesh mesh, XmlNode MeshNode, XmlDocument xDoc, string path)
	{
		...
		print (mesh.vertices + " number of vertices");
		print (mesh.sources.Length + " < length of sources");
		print (mesh.sources[0] + " < mesh.sources 1st array");
		//print (mesh.sources[1] + " < 2nd in array");
		XmlNode VertsNode = WriteVertices (mesh.vertices, mesh.sources[0].id, mesh.sources[0].id, xDoc);// changed 1 to 0 to test
